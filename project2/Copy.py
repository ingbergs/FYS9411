        ...
        for i in range(NumberParticles):
            for j in range(Dimension):
                if(importance):
                    ...
                    PositionNew[i,j] = PositionOld[i,j]+ step*(random()-0.5)
...
while iter < MaxIterations:
    ...
    iter += 1
f = open('ETAandTSfor1P1D_Brute_copy.txt', 'a')
f.write('Energies   eta\n')
for i in range(len(Energies)):
    f.write(str(Energies[i]) + '   ' + str(eta) + '\n') 
f.close()
"""
